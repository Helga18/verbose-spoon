...
public class CommentDAO {
    ...
    public List<Comment> getAllTrackComments(int id){
        try{
            PreparedStatement ps = connection.prepareStatement("select * from comment where track_id=? order by date DESC ");
            ps.setInt(1, id);
    ...
    public void addComment(String text, int user_id, int track_id) {
        try {
            PreparedStatement ps = connection.prepareStatement("insert into comment(text, date, user_id, track_id ) values (?,'now',?,?)");
            ps.setString(1, text);
            ps.setInt(2, user_id);
